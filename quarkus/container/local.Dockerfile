...
RUN --mount=type=secret,id=somansa_root_ca,required=false,target=/etc/pki/ca-trust/source/anchors/somansa_root_ca.crt \
    if [ -f /etc/pki/ca-trust/source/anchors/somansa_root_ca.crt ]; then update-ca-trust extract; fi && \
