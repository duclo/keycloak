...
public class UserStorageSyncManager {
    ...
    public static void bootstrapPeriodic(final KeycloakSessionFactory sessionFactory, final TimerProvider timer) {
		logger.info("******UserStorageSyncManager-bootstrapPeriodic()-59-ENABLED_AUTO_SYNC_JOB="
				+ System.getenv("ENABLED_AUTO_SYNC_JOB"));
		if (Boolean.parseBoolean(System.getenv("ENABLED_AUTO_SYNC_JOB"))) {
			KeycloakModelUtils.runJobInTransaction(sessionFactory, new KeycloakSessionTask() {

				@Override
				public void run(KeycloakSession session) {
					Stream<RealmModel> realms = session.realms()
							.getRealmsWithProviderTypeStream(UserStorageProvider.class);
					realms.forEach(realm -> {
						Stream<UserStorageProviderModel> providers = ((StorageProviderRealmModel) realm)
								.getUserStorageProvidersStream();
						providers.forEachOrdered(provider -> {
							UserStorageProviderFactory factory = (UserStorageProviderFactory) session
					...
					clusterProvider.registerListener(USER_STORAGE_TASK_KEY,
							new UserStorageClusterListener(sessionFactory));
				}
			});
		}
    }
